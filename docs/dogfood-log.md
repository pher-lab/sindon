...
- **✅ FW-27 [fw] P3 🧷 — テキスト入力は常に focus-visible(G7 の後追い精緻化)= 完了 (2026-07-08, gate 全緑・実機 OK)**
  出所: FW-26(G7)landed 後のユーザの気付き — 「この type の focus(border 色変化=web の `focus:border-*`)は**クリックでも表示される**印象がある。shroud も倣うべきか?」。調査で確定: web の `:focus-visible` は**一律ではない**。`<input>`/`<textarea>` の**テキスト入力欄はマウスクリックでも常に `:focus-visible` にマッチ**(キャレット位置を見せる必要がある)。Button/link 等の**コマンド系だけ keyboard 由来に限定**。FW-8 で入れた shroud の `:focus-visible` ゲートは**全 widget 一律**でクリック抑止していた ∴ テキスト入力について web とズレ = **G7 の記述(167–168 行「クリック focus では枠が変わらない挙動は保たれる」)を訂正する後追い**。
...
- **✅ FW-31 [fw] P2 — メニューを別ボタンのクリックで閉じた後、マウスを動かすまでその下のボタンに hover が出ない = 完了 (2026-07-10, 実機 OK)**
  ...
  真因確定: hover chain (`self.hovered`) を更新する経路が **live な `MouseMove` 一本しかない**([tree.rs](../crates/shroud_widgets/src/tree.rs) `update_hover_in`)。他に hover を触るのは `clear_hover`(None にするだけ)。∴ G15 で入れた「push 時に `clear_hover`」は正しいが**対の pop 側が無い**。カーソルが静止したまま layer が消えると、下の widget に誰も enter を撒かない。**G15 と同じ穴の残り半分**。
...
- **✅ FW-32 [fw] P2 — ダブルクリックで単語選択 → 離さずドラッグすると最初の単語が選択から外れる = 修正完了 (2026-07-11, 実機確認待ち)**
