...
use sindon_core::{Color, Colors, Point, Rect, Theme};
use sindon_platform::{PlatformWindow, SecureClipboard, SystemTheme};
use sindon_reactive::{Reactive, Signal};
use sindon_render::renderer::Renderer;
use sindon_security::hardening;
use sindon_widgets::event::{
    EventContext, ImeNav, Key, Modifiers, MouseButton, NamedKey, WidgetEvent,
};
use sindon_widgets::paint::PaintContext;
use sindon_widgets::shortcut::{Shortcut, ShortcutContext, ShortcutId};
...
///
/// Mirrors [`sindon_widgets::shortcut::ShortcutContext`]: the tick reaches
...
/// Thread-safe handle to a running sindon app.
///
...
impl AppScope {
    ...
    /// navigation. See [`sindon_widgets::shortcut`] for the scope rules
    ...
    /// [`sindon_platform::system_locale()`] so apps can read it through
...
struct AppConfig {
    ...
    /// [`TextEngine::load_font_data`](sindon_text::TextEngine::load_font_data).
    ...
    /// [`TextEngine::set_default_font_family`](sindon_text::TextEngine::set_default_font_family),
    ...
    /// `App::perf_overlay(true)` asks for it or `SINDON_HUD` is set in the
...
impl Default for AppConfig {
    fn default() -> Self {
        Self {
            ...
            // typing in every sindon app. Security-strict deployments opt in
            ...
            // touching its source — the same reflex as `SINDON_PERF`.
            perf_overlay: std::env::var_os("SINDON_HUD").is_some_and(|v| v != "0"),
        }
    }
}

/// Fluent builder for a sindon application.
///
...
impl App {
    ...
    /// Window title. Defaults to `"sindon"`.
    ...
    /// Korean typing in every sindon app. Opt in explicitly for flows that
    ...
    ///
    /// Off by default, and also switchable per *run* by setting `SINDON_HUD`
    ...
    ///
    /// Like the `SINDON_PERF` log, the readout is counts and durations only
        ...
        F: FnOnce(&mut AppScope) -> WidgetTree,
    {
        ...
        let mut handler = SindonEventLoop {
            config: self.config,
            handle,
            window: None,
            renderer: None,
...
/// Translate a winit named key into the corresponding sindon event.
///
...
struct SindonEventLoop {
    ...
    /// [`sindon_reactive::animation::request_frame_at`], captured after the
    ...
    /// Frame perf log, enabled by setting `SINDON_PERF` in the environment
...
impl SindonEventLoop {
    ...
    /// Arm the perf log's input→present timer (no-op unless `SINDON_PERF` is
...
impl ApplicationHandler<AppEvent> for SindonEventLoop {
    ...
    fn resumed(&mut self, event_loop: &ActiveEventLoop) {
        ...
        let caret_blink = match sindon_platform::caret_blink_time() {
            Some(interval) => sindon_widgets::caret::CaretBlink::Interval(interval),
            None => sindon_widgets::caret::CaretBlink::Off,
        };
        sindon_widgets::caret::set_caret_blink_from_system(caret_blink);
    ...
    fn window_event(
        ...
        match event {
            ...
            WindowEvent::RedrawRequested => {
                ...
                sindon_reactive::animation::reset_frame_request();
                ...
                // One perf-log line per painted frame (`SINDON_PERF`), plus a
                ...
                if sindon_reactive::animation::frame_requested() {
                    self.request_redraw();
                }
                ...
                self.anim_wake_at = sindon_reactive::animation::frame_deadline();
            }
