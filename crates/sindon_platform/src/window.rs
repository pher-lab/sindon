...
impl PlatformWindow {
    ...
    ///
    /// sindon lays out and paints in *logical* pixels: a widget authored as
    ...
    /// pixels — the render surface (see `sindon_render`) and the OS
    /// accessibility tree (see `sindon_app::a11y`). Read it fresh each frame
    ...
    ///
    /// sindon enables IME globally on window create (see `event_loop::resumed`)
    ...
    /// `sindon_app`) is left at its default (`false`); turning that on
    ...
    ///
    /// sindon calls this once per paint when a text widget is focused —
