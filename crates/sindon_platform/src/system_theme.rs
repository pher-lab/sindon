...
//! - Reactive subscription via `sindon_app::AppScope::system_theme`,
...
//! responsibility — sindon only reports what the OS prefers.

use winit::window::Theme as WinitTheme;

/// OS-reported UI theme preference. Mirrors winit's `Theme`, but kept
/// in `sindon_platform` so apps can match on it without importing
/// winit.
...
impl SystemTheme {
    /// Convert the winit-native enum into the sindon-facing one. Pure
