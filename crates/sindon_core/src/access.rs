...
//! (via `Widget::accessibility`); a single translation layer in `sindon_app`
//! turns the resulting snapshot into an `accesskit::TreeUpdate`. Nothing below
//! `sindon_app` links `accesskit`, mirroring how `winit` stays pinned to the
//! platform edge.
//!
//! # Secret safety
//!
...
///
/// Translated from `accesskit::Action` at the `sindon_app` edge and routed to
