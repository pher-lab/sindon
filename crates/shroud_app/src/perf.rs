...
//!
//! shroud paints on demand — a frame happens because an event arrived, an
...
//! widget names — the same rule the `SHROUD_PERF` log follows, so turning
...
/// per painted frame, from which both the HUD and the `SHROUD_PERF` log's
...
/// A closed one-second window, as handed to the `SHROUD_PERF` log.
