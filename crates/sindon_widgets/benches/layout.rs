...
//! `SINDON_PERF` log — every shaping call was a cache hit). That rules out
...
use sindon_widgets::{Container, Input, TextWidget};
...
fn bench_edit_keystroke(c: &mut Criterion) {
    let attrs = sindon_text::TextAttrs::default();
...
fn highlight_tiling(text: &str) -> Vec<sindon_text::TextSpan> {
    let accent = sindon_core::Color::rgb(0.85, 0.4, 0.2);
    text.split_inclusive(' ')
        .enumerate()
        .map(|(i, chunk)| {
            let span = sindon_text::TextSpan::new(chunk);
