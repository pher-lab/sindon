...
use sindon_core::{Color, Point, SecurityLevel};
...
fn secure_input_char_input() {
    ...
    let rect = sindon_core::Rect::new(0.0, 0.0, 200.0, 40.0);
...
fn focused_secure_input(text: &str) -> (SecureInput, EventContext, sindon_core::Rect) {
    let mut input = SecureInput::new();
    let mut ctx = EventContext::new();
    let rect = sindon_core::Rect::new(0.0, 0.0, 200.0, 40.0);
    input.event(&WidgetEvent::FocusGained, rect, &mut ctx);
...
fn press(input: &mut SecureInput, ctx: &mut EventContext, rect: sindon_core::Rect, key: NamedKey) {
    input.event(
        &WidgetEvent::KeyDown {
            key: Key::Named(key),
        },
...
fn type_char(input: &mut SecureInput, ctx: &mut EventContext, rect: sindon_core::Rect, ch: char) {
...
fn secure_input_border_mode_recolors_border_and_omits_ring() {
    ...
    use sindon_core::{FocusIndicator, Theme};
...
const REVEAL_RECT: sindon_core::Rect = sindon_core::Rect {
