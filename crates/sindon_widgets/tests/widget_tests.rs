use sindon_core::{Color, FocusIndicator, Point, Theme};
...
fn centered_card_with_button_hugs_content_height() {
    ...
    fn card_and_button_at(scale: f32) -> (sindon_core::Rect, sindon_core::Rect, f32) {
        let mut tree = WidgetTree::new();
        let root = tree.set_root(
            Container::column()
                .width_full()
...
fn centered_card_with_dropdown_and_menu_item_hug_content_height() {
    ...
    fn card_and_last_child<F>(
        scale: f32,
        add_last: F,
    ) -> (sindon_core::Rect, sindon_core::Rect, f32)
    where
...
fn container_justify_space_between_positions_children_at_the_ends() {
    ...
    let root = tree.set_root(
        Container::row()
            .width(400.0)
            .height(40.0)
            .justify(sindon_layout::Justify::SpaceBetween),
    );
    tree.add_child(
        root,
        Container::column()
...
fn container_justify_align_are_chainable() {
    let _c = Container::row()
        .justify(sindon_layout::Justify::End)
        .align(sindon_layout::Align::Start)
...
// fill, then a 1px border stroke. `.radius()` rounds both, `.borderless()`
// drops the stroke, `.border_color()` recolors it.

fn paint_input(input: Input) -> PaintContext {
    let mut tree = WidgetTree::new();
...
#[test]
fn input_radius_rounds_both_fill_and_border() {
    let ctx = paint_input(Input::new().radius(10.0));
    assert_eq!(ctx.rects.len(), 2);
...
#[test]
fn input_negative_radius_clamps_to_zero() {
    let ctx = paint_input(Input::new().radius(-3.0));
    assert_eq!(ctx.rects[0].radius, 0.0);
}

#[test]
fn input_borderless_drops_the_stroke() {
    let ctx = paint_input(Input::new().borderless());
...
#[test]
fn input_border_color_overrides_the_stroke() {
    let red = Color::rgb(1.0, 0.0, 0.0);
    let ctx = paint_input(Input::new().border_color(red));
    let stroke = ctx
        .rects
        .iter()
...
fn focused_caret_blinks_on_a_timed_cycle_and_resets_on_typing() {
    ...
    use sindon_reactive::animation::test_clock;
    use sindon_widgets::caret::{CaretBlink, set_caret_blink};
...
fn scroll_view_scroll_updates_offset() {
    ...
    let layout = sindon_core::Rect::new(0.0, 0.0, 200.0, 300.0);
...
fn scroll_view_no_scroll_when_content_fits() {
    ...
    let layout = sindon_core::Rect::new(0.0, 0.0, 200.0, 300.0); // viewport bigger
    let mut ectx = EventContext::new();
    let event = WidgetEvent::Scroll {
        position: Point::new(50.0, 50.0),
        delta_x: 0.0,
...
fn text_widget_monospace_normalizes_iii_and_mmm_widths() {
    ...
    // ratio test mirrors `sindon_text`'s `monospace_family_makes_iii_and_
...
impl sindon_widgets::Widget for DropCounter {
    fn style(&self) -> sindon_layout::FlexStyle {
        sindon_layout::FlexStyle::new().width(10.0).height(10.0)
    }
    fn paint(&self, _layout: sindon_core::Rect, _ctx: &mut PaintContext) {}
}
...
fn secure_input_drops_when_removed_from_tree() {
    // SecureString zeroize-on-drop is covered in sindon_security's own tests.
...
fn input_cursor_signal_mirrors_caret() {
    ...
    tree.dispatch_event(
        &key(sindon_widgets::event::NamedKey::ArrowLeft),
        &mut event_ctx,
    );
...
impl sindon_widgets::Widget for FocusProbe {
    ...
    fn paint(&self, _: sindon_core::Rect, _: &mut PaintContext) {}
    fn event(
        &mut self,
        event: &WidgetEvent,
        _: sindon_core::Rect,
...
fn tab_order_skips_invisible_subtrees() {
    ...
    let hidden_parent = tree.add_child(
        root,
        Container::column().visible(sindon_reactive::Reactive::from(false)),
    );
...
fn button_is_focusable_by_default() {
    ...
    assert!(<Button as sindon_widgets::Widget>::focusable(&btn));
}

#[test]
fn checkbox_is_focusable_by_default() {
    let cb = Checkbox::new("Accept");
    assert!(<Checkbox as sindon_widgets::Widget>::focusable(&cb));
}
...
fn event_context_focus_command_dispatches_on_drain() {
    ...
    tree.dispatch_event(
        &WidgetEvent::MouseMove {
            position: sindon_core::Point::new(-1.0, -1.0),
        },
        &mut ctx,
    );
...
impl sindon_widgets::Widget for HoverProbe {
    fn style(&self) -> sindon_layout::FlexStyle {
        sindon_layout::FlexStyle::new().width(40.0).height(40.0)
    }
    fn paint(&self, _: sindon_core::Rect, _: &mut PaintContext) {}
    fn event(
        &mut self,
...
fn hover_fade_default_animates_and_requests_frames() {
    ...
    use sindon_reactive::animation::{frame_requested, reset_frame_request, test_clock};
...
fn key(named: sindon_widgets::event::NamedKey) -> WidgetEvent {
    WidgetEvent::KeyDown {
        key: sindon_widgets::event::Key::Named(named),
    }
}
...
fn multiline_enter_inserts_newline_into_signal() {
    ...
    tree.dispatch_event(&key(sindon_widgets::event::NamedKey::Enter), &mut ctx);
...
fn multiline_enter_does_not_fire_on_submit() {
    ...
    tree.dispatch_event(&key(sindon_widgets::event::NamedKey::Enter), &mut event_ctx);
...
fn single_line_arrow_up_down_are_no_ops() {
    ...
    tree.dispatch_event(&key(sindon_widgets::event::NamedKey::ArrowUp), &mut ctx);
    tree.dispatch_event(&key(sindon_widgets::event::NamedKey::ArrowDown), &mut ctx);
...
fn multiline_does_not_intercept_tab() {
    ...
    tree.dispatch_event(&key(sindon_widgets::event::NamedKey::Tab), &mut event_ctx);
    assert_eq!(
        tree.focused(),
        Some(second),
...
fn shortcut_fires_through_tree_with_no_focus() {
    use sindon_widgets::event::{Key, Modifiers, WidgetEvent};
