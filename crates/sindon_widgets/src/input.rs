...
use sindon_core::{AccessNode, AccessRole, Color, FocusIndicator, Lerp, Point, Rect};
use sindon_layout::FlexStyle;
use sindon_reactive::animation;
use sindon_reactive::{Animated, Easing, Reactive, Signal};
use sindon_text::{FontWeight, TextAttrs, TextSpan};
...
/// `sindon_text/tests/highlight_layout_spike.rs`), so the caret / selection /
...
impl Input {
    ...
    fn resolve_bg(&self, colors: &sindon_core::Colors) -> Color {
        self.bg_color
            .as_ref()
            .map(|c| c.get())
    ...
    fn resolve_border(&self, colors: &sindon_core::Colors) -> Color {
        self.border_color
            .as_ref()
            .map(|c| c.get())
    ...
    fn focus_indicator_color(&self, focus: &sindon_core::FocusStyle) -> Color {
...
impl Widget for Input {
    ...
    fn paint(&self, layout: Rect, ctx: &mut PaintContext) {
        ...
        // IME composition underlines (the modern inline-composition affordance,
        // matching Chromium / macOS / Win11 IMEs rather than the legacy IMM32
        // reverse-video block): a thin, dimmed rule under the whole preedit marks
        // it as uncommitted, and a thick rule under the *target clause* (注目文節)
        // marks the segment `space` is currently converting. Both reuse the
        // block's `selection_rects`-derived rects (one per visual row) and sit at
        // the row bottom, inside the multi-line clip/offset so they scroll with
        // the glyphs. The thick target rule is drawn last so it fully covers the
        // thin one where they overlap.
        if let Some(block) = composed_block.as_ref() {
            let preedit_rule = Color {
                a: text_color.a * 0.5,
                ..text_color
            };
            ...
            for r in &block.target {
                ctx.fill_rect(
                    Rect::new(
                        text_x + r.origin.x,
                        text_y + r.origin.y + r.size.height - 2.0,
                        r.size.width,
                        2.0,
                    ),
                    text_color,
        ...
        if let Some((cx, cy)) = caret_xy {
            ...
            // While converting, the target clause is shown with a thick underline
            // (above), so a thin caret would just sit at the clause head marking
            // nothing — suppress its fill. Keyed off the target rule actually
