...
use sindon_core::{Color, Point, Rect, Theme};
use sindon_render::{DecodedImage, DrawGlyph, DrawImage, DrawRect, GlyphRotation, LayerSnapshot};
use sindon_text::{GlyphImage, TextEngine};
...
impl PaintContext {
    ...
    pub fn draw_glyph(
        ...
        image: GlyphImage,
        color: Color,
        cache_key: sindon_text::CacheKey,
    ) {
        let (ox, oy) = self.current_offset();
    ...
    /// `sindon_text`). That keeps a run crisp *only while its origin also sits
