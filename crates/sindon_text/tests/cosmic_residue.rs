...
//! because shaping goes through sindon's vendored fork of cosmic-text, whose
...
//!
//! This scanner is duplicated in `crates/sindon_widgets/tests/zeroize_verification.rs`
...
///    duplicate in `sindon_widgets` — that binary has two scanning tests).
...
fn cosmic_text_residue_after_drop() {
    ...
    // of plaintext may survive on the heap. This holds because sindon vendors a
    // fork of cosmic-text whose `BufferLine` zeroizes its text on drop (see
    // `third_party/cosmic-text` and sindon_text's crate docs). Before the fork
